<script lang="ts" setup></script>

<template>
  <div class="w-full bg-secondary" id="story">
    <div class="mx-auto min-w-64 max-w-[60%] justify-center py-12">
      <p class="mb-2 text-4xl font-bold text-primary">
        Miguel Aguas Claras Studio
      </p>
      <p class="mb-4 text-2xl font-semibold text-platinum">
        Content Creators Paradise
      </p>
      <p class="text-white">
        Lorem, ipsum dolor sit amet consectetur adipisicing elit. Excepturi
        tempora incidunt labore earum! Debitis nihil ad cumque architecto
        placeat illo eius explicabo pariatur, vero autem iure facere nemo
        aperiam? Aliquam?
      </p>
    </div>
  </div>
</template>

<style scoped></style>
